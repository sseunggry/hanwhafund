...
const commonJs = {
	init: function () {
		...
		$(`.${projectName}-accordion`).each(commonJs.accordion);
		$(`.${projectName}-tab-area`).each(commonJs.tabs);
	...
	bindGlobalEvents: function () {
		...
		$(document).on('click', `.${projectName}-accordion .btn-accordion`, function (e) {
			e.preventDefault();
			// 클릭된 버튼이 속한 아코디언 그룹을 찾아 accordionToggle 호출
			const $accordionWrapper = $(this).closest(`.${projectName}-accordion`);
			if ($accordionWrapper.length) {
				// 'this'를 클릭된 버튼으로 전달
				commonJs.accordionToggle.call(this, $accordionWrapper);
			}
		});

		$(document).on('click', `.${projectName}-tab-area [role="tab"]`, function(e) {
			e.preventDefault();
			const $tabArea = $(this).closest(`.${projectName}-tab-area`);
			if ($tabArea.length) {
					// 'this'를 클릭된 탭(li)으로 전달하여 activateTab 호출
					commonJs.activateTab.call(this, $tabArea);
			}
	});
	},
	resize: function () {
	...
	modal: {
		activeModals: {},
		defaultSettings: {
			activeClass: 'show in', // 'show' 대신 'active' 사용 권장 (일관성)
	...
	accordion: function () { // 초기화 함수 ('this'는 아코디언 루트 div)
		const $accordionWrapper = $(this);
		const accordionType = $accordionWrapper.data('type') || 'singleOpen'; // 타입 읽기

		// 중복 초기화 방지 (선택적)
		if ($accordionWrapper.data('accordion-initialized')) return;
		$accordionWrapper.data('accordion-initialized', true);

		// (초기화 시 특별히 할 작업이 없다면 비워둘 수 있음)
		// console.log(`Accordion initialized: ${accordionType}`, this);

		// 키보드 네비게이션 등 추가 기능은 여기에 구현 가능
	},

	accordionToggle: function ($accordionWrapper) { // 토글 함수 ('this'는 클릭된 버튼)
		const $btn = $(this);
		...
		if ($btn.is(':disabled')) return; // 비활성화 버튼 무시

		if (accordionType === 'singleOpen') {
			// --- Single Open 로직 ---
			if (isActive) {
				// 이미 열린 항목 클릭 시: 닫기
				$item.removeClass('active');
				$collapse.slideUp(200).attr('hidden', true); // 애니메이션 후 hidden
				$btn.attr('aria-expanded', 'false');
			} else {
				// 다른 열린 항목 찾아서 닫기
				$accordionWrapper.find('.accordion-item.active').each(function () {
					...
					$otherCollapse.slideUp(200).attr('hidden', true);
					$otherBtn.attr('aria-expanded', 'false');
				});

				// 현재 항목 열기
				$item.addClass('active');
				$collapse.slideDown(200).removeAttr('hidden'); // hidden 제거 후 애니메이션
				$btn.attr('aria-expanded', 'true');
			}
		} else {
			// --- Multi Open 로직 ---
			$item.toggleClass('active');
			$collapse.slideToggle(200); // jQuery slideToggle 사용
			// hidden 속성 토글 (애니메이션 완료 후 하는 것이 더 좋음)
			$collapse.promise().done(function () {
				if ($item.hasClass('active')) {
					$collapse.removeAttr('hidden');
				} else {
					$collapse.attr('hidden', true);
				}
			});
			$btn.attr('aria-expanded', !isActive); // ARIA 상태 토글
		}
	},

	tabs: function() { // 초기화 함수 ('this'는 탭 영역 div)
    const $tabArea = $(this);
    ...
    // 초기 상태 확인 및 ARIA 설정 (EJS에서 이미 처리했으므로 생략 가능, 안전장치로 추가)
    const $activeTab = $tabs.filter('.active');
    if ($activeTab.length === 0 && $tabs.length > 0) {
        // EJS에서 active 설정이 안된 경우 첫 탭 활성화 (선택적)
        // commonJs.activateTab.call($tabs.first()[0], $tabArea);
    } else {
        // aria-selected 동기화
        $tabs.each(function(){
            $(this).attr('aria-selected', $(this).hasClass('active'));
        });
        // aria-hidden 또는 hidden 속성 동기화 (CSS에서 .active로 제어한다면 불필요)
        $panels.each(function(){
            const isActive = $(this).hasClass('active');
             // $(this).attr('aria-hidden', !isActive);
             if(!isActive) $(this).attr('hidden', true); // CSS 제어가 아니면 hidden 속성 관리
        });
    }

    console.log('Tabs initialized for:', this);
  }, // end tabs init

  activateTab: function($tabArea) { // 탭 활성화 함수 ('this'는 클릭된 탭 li)
      const $clickedTab = $(this);
      if ($clickedTab.hasClass('active') || $clickedTab.is('[aria-disabled="true"]')) {
          return; // 이미 활성이거나 비활성이면 무시
      }
      ...
      // $tabs.find('.sr-only').remove(); // ' 선택됨' 텍스트 제거 (필요 시)

      // 2. 모든 패널 숨기기 (CSS 또는 hidden 속성)
      $panels.removeClass('active').attr('hidden', true);
      // $panels.attr('aria-hidden', 'true');

      // 3. 클릭된 탭 활성화
      $clickedTab.addClass('active').attr('aria-selected', 'true');
      // $clickedTab.find('.btn-tab').append('<span class="sr-only"> 선택됨</span>'); // ' 선택됨' 텍스트 추가 (필요 시)

      // 4. 해당 패널 보이기
      const $targetPanel = $panels.filter('#' + targetPanelId);
