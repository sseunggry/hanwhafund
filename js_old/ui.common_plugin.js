(function ($) {
	...
	// 위치 계산 함수
	function _toggleUpdatePosition($btn, $tooltipEl, settings) {
	...
	// 툴팁 열기 로직
	function _toggleOpen($btn, $targetEl, settings) {
		...
		_toggleUpdatePosition($btn, $targetEl, settings); // 위치 계산
		...
		setTimeout(() => { // 외부 클릭 닫기 바인딩
			$(document).on(eventNamespace, function (e) {
				if ($(e.target).closest($targetEl).length === 0 && !$(e.target).is($btn) && $(e.target).closest($btn).length === 0) {
					_toggleClose($targetEl, $btn, settings); // 내부 닫기 함수 호출
				}
			});
		}, 0);
	}
	...
	// 툴팁 닫기 로직
	function _toggleClose($tooltipToClose, $btn, settings) {
				...
				switch (methodName) {
					case 'open':
						if ($btn.length && $tooltipEl && $tooltipEl.length) {
							_toggleOpen($btn, $tooltipEl, settings);
						}
						break;
					case 'close':
						if ($tooltipEl && $tooltipEl.length) {
							_toggleClose($tooltipEl, $btn, settings);
							...
							if ($closestTooltip.length) {
								...
								_toggleClose($closestTooltip, $closeOpener, settings);
							}
						}
						break;
					case 'toggle':
					...
					case 'updatePosition':
						if ($btn.length && $tooltipEl && $tooltipEl.length && $tooltipEl.hasClass(settings.activeClass)) {
							_toggleUpdatePosition($btn, $tooltipEl, settings);
						}
						break;
				}
			});
			...
			if (targetId) {
				...
				if ($targetEl.length) {
					...
					if (isActiveInitially) { // 이미 active 클래스가 있으면:
						...
						_toggleUpdatePosition($btn, $targetEl, settings);
						...
						setTimeout(() => {
							...
							$(document).on(eventNamespace, function (e) {
								if (
									...
									_toggleClose($targetEl, $btn, settings);
								}
							});
						}, 0);
