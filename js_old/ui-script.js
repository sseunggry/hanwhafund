...
const krds_pcGnb = {
  init() {
    ...
    // role 초기설정 // gnb pc roll
    // gnbMenu.setAttribute("role", "menubar");
    // gnbMenu.querySelectorAll("li").forEach((li) => li.setAttribute("role", "none"));
    // gnbMenu.querySelectorAll("ul").forEach((ul) => ul.setAttribute("role", "none"));
    // gnbMenu.querySelectorAll(".single-list li button, .single-list li a").forEach((item) => item.setAttribute("role", "menuitem"));
  ...
  setupMainTrigger(mainTrigger) {
    // mainTrigger.setAttribute("role", "menuitem"); // gnb pc roll
    ...
    if (toggleWrap) {
      ...
      // toggleWrap.setAttribute("role", "menu"); // gnb pc roll
  ...
  setupSubTrigger(subTrigger) {
    // subTrigger.setAttribute("role", "menuitem"); // gnb pc roll
    const hasMenu = subTrigger.nextElementSibling;
    if (hasMenu && !subTrigger.classList.contains("active")) {
      ...
      // hasMenu.setAttribute("role", "menu"); // gnb pc roll
      // 3depth
      // const lastTriggers = hasMenu.querySelectorAll(".gnb-sub-content li button, .gnb-sub-content li a");
      // lastTriggers.forEach((lastTrigger) => {
      //   lastTrigger.setAttribute("role", "menuitem");
      // })
    }
  },
...
const krds_tab = {
  ...
  init() {
    ...
    if (this.layerTabArea.length === 0) return;
...
const krds_accordion = {
  ...
  init() {
    ...
    if (this.accordionButtons.length === 0) return;
...
const krds_modal = {
  ...
  init() {
    ...
    if (this.modalOpenTriggers.length === 0 || this.modalCloseTriggers.length === 0) return;
...
const krds_contextualHelp = {
  tooltipButtons: null,
  init() {
    this.tooltipButtons = document.querySelectorAll(".krds-tooltip-wrap .tooltip-btn");

    if (this.tooltipButtons.length === 0) return;
  ...
  setupTooltips() {
    this.tooltipButtons.forEach((button) => {
      const tooltipContainer = button.closest(".krds-tooltip-wrap");
  ...
  closeAllTooltips() {
    const otherPopovers = document.querySelectorAll(".krds-tooltip-wrap .tooltip-popover");
  ...
  adjustTooltipPosition(tooltipContainer, tooltipPopover) {
    ...
    if (isMobile) {
      const offsetLeft = tooltipAction.offsetLeft - 16;
  ...
  setupFocusOutEvent() {
    document.addEventListener("click", (event) => {
      ...
      if (!clickedInsideTooltip) {
        ...
        const FocusButtons = clickedInsideTooltip.querySelectorAll(".tooltip-popover .krds-btn");
        if (FocusButtons) {
          const lastFocus = FocusButtons[FocusButtons.length - 1];
...
const krds_calendar = {
  ...
  init() {
    ...
    if (this.datePickerArea.length === 0) return;
  ...
  setTableHeight(currentDatePicker) {
    const cells = currentDatePicker.querySelectorAll(".datepicker-tbl td");
    cells.forEach((cell) => {
      const cellWidth = (cell.clientWidth + 4).toFixed(2);
  ...
  openDatePicker(button) {
    ...
    this.setTableHeight(currentDatePicker);
...
const krds_inPageNavigation = {
  ...
  init() {
    ...
    if (this.quickIndicators.length === 0) return;
  ...
  observeTabChanges() {
    // 탭 변경 시 setupAnchorScroll 호출
    const isTabArea = document.querySelector(".krds-quick-nav-type .tab-area");
    if (!isTabArea) return;
    const observer = new MutationObserver(() => {
      this.setupAnchorScroll();
    });
    observer.observe(document.querySelector(".tab-area"), {
      childList: true,
      subtree: true,
    });
  },
  ...
  applyScroll(target, event) {
    event.preventDefault();
    const headerHeight = (document.querySelector("#header-top")?.clientHeight || 0) + (document.querySelector("#header .header-in")?.clientHeight || 0);

    window.scrollTo({
      left: 0,
      top: target.getBoundingClientRect().top - headerHeight + window.scrollY,
      behavior: "smooth",
    });

    // enter 초점 이동
    if (event.type === "keydown") {
      target.querySelector(".sec-tit")?.setAttribute("tabindex", "-1");
      target.querySelector(".sec-tit")?.focus({ preventScroll: true });
    }
  },
  updateActiveSection() {
...
const krds_adjustContentScale = {
  ...
  maxScale: 2,
  init() {
    const scaleButtons = document.querySelectorAll(".zoom-drop .item-link, .zoom-drop .krds-btn");

    if (scaleButtons.length === 0) return;

    // root 변수에서 스케일 값을 가져오는 경우 테스트
    const root = document.querySelector(":root");
    const rootStyles = getComputedStyle(root);
    const scaleTest = rootStyles.getPropertyValue("--krds-space-8").trim().slice(0, 3); // 1.6
    ...
    scaleButtons.forEach((button) => {
      button.addEventListener("click", () => {
        switch (true) {
          case button.classList.contains("xsm"):
            this.scaleValue(0.9);
            break;
          case button.classList.contains("sm"):
            this.scaleDefault();
            break;
          case button.classList.contains("medium"):
            const others = button.closest(".drop-menu").querySelectorAll(".item-link");
            const defaultSize = button.closest(".drop-menu").querySelector(".item-link.sm");
            others.forEach((item) => {
            ...
            this.scaleDefault();
            break;
          case button.classList.contains("md"):
            this.scaleValue(1.1);
            break;
          case button.classList.contains("lg"):
            this.scaleValue(1.2);
            break;
          case button.classList.contains("xlg"):
            this.scaleValue(scaleTest);
            break;
          default:
            break;
        }
...
/*** * krds_dropEvent(gnb utils / page-title-wrap) 영역 높이 확장 축소 * ***/ // 1차수정
const krds_dropEvent = {
  dropButtons: null,
  init() {
    this.dropButtons = document.querySelectorAll(".krds-drop-wrap:not(.sample) .drop-btn");

    if (this.dropButtons.length === 0) return;

    this.dropOpen();
    this.setupFocusOutEvent();
  },
  dropList(menu) {
    const lists = menu.querySelectorAll(".item-link");

    lists.forEach((list) => {
      list.innerHTML += `<span class="sr-only"></span>`;
      if (list.classList.contains("active")) {
        list.querySelector(".sr-only").innerHTML = "선택됨";
      }

      list.addEventListener("click", () => {
        const otherLists = list.closest(".drop-menu").querySelectorAll(".item-link");
  ...
  dropOpen() {
    this.dropButtons.forEach((button) => {
      ...
      const focusTarget = menu.querySelectorAll("a, button");
      button.setAttribute("aria-expanded", "false");
  ...
  dropClose() {
    const dropMenus = document.querySelectorAll(".krds-drop-wrap .drop-menu");
  ...
  setupFocusOutEvent() {
    document.addEventListener("click", (event) => {
      const dropWrap = event.target.closest(".krds-drop-wrap");
      if (!dropWrap) {
        this.dropClose();
      } else {
        const dropMenu = dropWrap.querySelector(".drop-menu");
        const dropItems = dropWrap.querySelectorAll(".drop-menu a, .drop-menu button");
        if (dropItems.length > 0) {
          const firstItem = dropItems[0];
          const lastItem = dropItems[dropItems.length - 1];

          firstItem.addEventListener("focusout", (event) => {
            if (!event.relatedTarget || !dropMenu.contains(event.relatedTarget)) {
              this.dropClose();
            }
          });
...
// icon -> base64로 변경
async function iconToBase64(name, url) {
	...
	el.forEach((item) => {
		...
		item.style.maskImage = `url(${dataUrl})`;
	});
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (windowSize.getWinSize() === "pc") {
	...
	iconToBase64("ico-angle", "/resources/img/component/icon/ico_angle.svg");
});
